      ...
      <div className="card hover:bg-surface-hover hover:border-surface-active transition-all cursor-pointer h-full">
        ...
        <div className="flex flex-wrap gap-1">
          {space.channels.slice(0, 4).map((ch) => (
            <span key={ch.id} className="text-xs">{CHANNEL_META[ch.type].icon}</span>
          ))}
...
export default function DashboardPage() {
  ...
  const archivePosts = spaces.reduce((acc, s) => acc + s.channels.filter(c => c.type === 'archive').reduce((a, c) => a + c.postCount, 0), 0)

  function getChannelSlug(post: typeof posts[0]) {
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        ...
        <div className="flex-1 p-4 lg:p-6 max-w-4xl mx-auto w-full space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
            ...
            <StatCard label="Gönderi" value={posts.length > 0 ? posts.length : 0} icon={Flame} color="bg-accent-green/10 text-accent-green" isLoading={postsLoading} />
